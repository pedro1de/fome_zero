...
with st.sidebar:
    ...
    country_selected = st.selectbox("Selecione o país", countries)
...
with col2:
    avg_rating = df_country["rating"].mean() if "rating" in df_country.columns else None
    st.metric("⭐ Avaliação média", f"{avg_rating:.2f}" if avg_rating else "—")

with col3:
    if "price_num" in df_country.columns:
